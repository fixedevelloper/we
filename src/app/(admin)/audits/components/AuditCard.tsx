...
import { Card, CardHeader, Table, Dropdown, DropdownToggle, DropdownMenu, DropdownItem, CardFooter, Pagination, Modal, ModalHeader, Button, Form, ModalBody, ModalFooter } from "react-bootstrap";
import Link from "next/link";
import {useFetchData} from "../../../../hooks/useFetchData";
import API_ENDPOINTS from "../../../(other)/api/Constant";
import useModal from '@/hooks/useModal';
import AuditPdfModal from '../AuditPdf';
...
const AuditCard: React.FC = () => {
  ...
  const audits = data?.data?.results ?? [];
      ...
      <Card>
        <CardHeader className="border-bottom card-tabs d-flex flex-wrap align-items-center gap-2">
          <div className="flex-grow-1">
            <h4 className="header-title">Audits</h4>
          </div>
        ...
        <div className="table-responsive">
              ...
              <Table className="table-hover text-nowrap mb-0">
                ...
                {audits.map((t) => (
                    <tr key={t.id}>
                      ...
                      <td><span className="fs-15 text-muted">{t.action || "-"}</span></td>
          ...
          <ModalHeader onHide={toggleModal} closeButton>
            <h4 className="modal-title">Filtrer les audits</h4>
          </ModalHeader>
